...
# core/ir: plr_v1.4_cot is the reconciled live tuple and MUST pass; the two
# older rows document historical drift (superseded, kept as strict xfail).
_COMPARABLE = [
    ("plr_v1.4_cot", "yaml", "on"),   # the LIVE production tuple — reconciled
    pytest.param(
        "plr_v1.3_cot", "yaml", "on",
        marks=pytest.mark.xfail(reason="superseded by plr_v1.4_cot; historical drift", strict=True),
    ),
    pytest.param(
        "plr_v0.4", "json", "off",
        marks=pytest.mark.xfail(reason="legacy JSON constants/yaml diverged; v1.4 is the reconciled target", strict=True),
    ),
]
...
_QP_VERSIONS = ["plr_v0.4", "plr_v1.3_cot", "plr_v1.4_cot"]
