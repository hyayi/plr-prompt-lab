...
python3 -m pytest tests/ -q     # 기대: 111 passed, 4 xfailed (GPU·DB 불필요)
...
- `runners/` — re_score(크롭 루프)·demo·client(서버 제출) /
  `evalkit/` — scoring·dataset·validate·gallery·report·provenance (채점/렌더는 서버가 호출)
- `server/` — 평가 서버(FastAPI: 채점·리더보드·report/gallery 렌더·버전관리) /
