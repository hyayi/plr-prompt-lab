...
> 📖 **처음이라면 [`GUIDE.html`](GUIDE.html)을 브라우저로 여세요** — 구조와 사용법을
> 한 장(자체완결 HTML·오프라인)으로 그림과 함께 설명합니다.
> 아키텍처 시각화는 [`STRUCTURE.html`](STRUCTURE.html), 영문판은 [`README.md`](README.md).

PLR(Person-Level Recognition)의 **속성 채점(A)** 과 **텍스트 검색(B)** 을 위한
독립 프롬프트/파이프라인 평가 사이클 도구입니다. `core/ir` HEAD
`c2fc1cf20a6fbd3ad4272aec8439d438a4febf34`에서 lean 추출했습니다([SEED.md](SEED.md) 참고).
...
온보딩 `lab demo`, [DATASET_SPEC.md](DATASET_SPEC.md) 형식 스펙,
[HANDOFF.md](HANDOFF.md) 외부 프롬프트 엔지니어 가이드. v2 Phase 2에서
모델/파이프라인 **레지스트리**, **experiment 매트릭스 러너**, **HTML 리포트**가
추가로 구현됐습니다([EXPERIMENT_SPEC.md](EXPERIMENT_SPEC.md) 참고).
...
- `search_core.py`, `scoring.py`, `query_parser.py` — 순수 검색/채점
- `prompts/` — PLR 프롬프트 YAML (`plr_v0.4`, `plr_v1.3_cot`, `plr_v1.4_cot`)
...
| **파이프라인** | `--pipeline plr\|search` (레지스트리) | 구현됨 (P2-1) |
...
([EXPERIMENT_SPEC.md](EXPERIMENT_SPEC.md)). 결과 추이는 `lab report`가
자체완결 HTML로 만들어 줍니다.
...
### B — 텍스트 검색 평가 (recall@k)

```
┌─────────────────────────────────────────────────────────────────────┐
│  1. (build-golden / label은 위와 동일 — 같은 크롭 사용)                │
│                                                                     │
│  2. run  (attributes.jsonl도 함께 기록)                              │
│     lab run --attribute gender --version plr_v1.4_cot               │
│              --dataset <dataset>                                    │
│     → <dataset>/queries.jsonl  (사람이 직접 작성)                     │
│     → <dataset>/attributes.jsonl  (run이 생성)                       │
│     → <dataset>/search_results.jsonl  (run이 생성)                   │
│                                                                     │
│  3. eval --mode search                                              │
│     lab eval --attribute search --mode search --version plr_v1.4_cot│
│              --dataset <dataset>                                    │
│     → recall@k / precision@k + 이전 버전 대비 Δ 출력                  │
│     → eval/ledger.jsonl에 레코드 추가                                │
└─────────────────────────────────────────────────────────────────────┘
```
...
# Gemma로 재채점 (GPU 단계)
python3 lab.py run --attribute gender --version plr_v1.4_cot [--dataset <dir>] [--model gemma|mock] [--pipeline plr|search]
...
데이터셋 디렉터리는 [DATASET_SPEC.md](DATASET_SPEC.md)를 따라야 합니다.
...
[HANDOFF.md](HANDOFF.md)는 운영 서비스를 건드리지 않고 PLR/검색 프롬프트를
...
전체 셋업(Python 환경, CUDA 빌드, 모델 다운로드)은 [INSTALL.md](INSTALL.md) 참고.
...
### B — 텍스트 검색 메트릭 (`eval/ledger.jsonl` 레코드 필드)
...
A와 B 레코드는 **같은** `eval/ledger.jsonl` 파일에 쌓이며,
`(attribute, version)`으로 구분됩니다. experiment의 `formats:`/`reasons:` 축을
쓰면 version 태그에 `+json`/`+reason-off` 접미사가 붙어 셀이 구분됩니다.
...
| [GUIDE.html](GUIDE.html) | 구조+사용법 원페이지 (배포용) |
| [STRUCTURE.html](STRUCTURE.html) | 아키텍처 시각화 (계층·파이프라인·프롬프트 2-소스) |
| [HANDOFF.md](HANDOFF.md) | 외부 프롬프트 엔지니어 워크플로 |
| [DATASET_SPEC.md](DATASET_SPEC.md) | 데이터셋 디렉터리 형식 스펙 |
| [EXPERIMENT_SPEC.md](EXPERIMENT_SPEC.md) | experiment 매트릭스 yaml 스펙 |
| [INSTALL.md](INSTALL.md) | GPU/Gemma 실측 환경 셋업 |
