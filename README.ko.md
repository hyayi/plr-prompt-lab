...
v2: `--dataset` 파라미터, `validate-dataset`, `lab demo`,
[DATASET_SPEC.md](docs/DATASET_SPEC.md), [HANDOFF.md](docs/HANDOFF.md);
Phase 2에서 레지스트리·`lab experiment run`·`lab report`
([EXPERIMENT_SPEC.md](docs/EXPERIMENT_SPEC.md)). **최신(2026-07)**:
plr_v1.5_cot 강제커밋 동기화, precision/F1 + margin/quality 캘리브레이션,
`lab gallery` 시각 평가, manifest 선언식 generic 라벨,
improve-prompt agent 스킬.
